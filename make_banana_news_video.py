...
def generate_video_text_fun(input_video,output_video,text_file,batch_num):
    ...
    ffmpeg_str='ffmpeg -i '+input_video+' -vf '+'[in]'+font_setting_str+':fontfile='+font_txt_path+':textfile='+text_file+':x=100:y=h-'+number_str+' -y '+output_video
...
def move_final_video_set():
    #databe_path='/media/xiaolong/datapool/data/youtube/news/sina_news/2019-03-22/video/'
    databe_path='/media/xiaolong/datapool/data/youtube/news/sina_news/'+str(date.today())+'/video/'

    video_list=glob.glob(databe_path+'*banana_channel.mp4')
    for video_path in video_list:
        dst=video_path.replace('-banana_channel.mp4','.mp4').replace('/video/','/release_video/')
        if 1:#not os.path.exists(dst.replace('2019-03-19','2019-03-18')):
...
def generate_news_video_set():
    ...
    databe_path='/media/xiaolong/datapool/data/youtube/news/sina_news/2019-05-08/json/'
