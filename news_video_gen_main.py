...
def generate_mp3_function(news_str,audio_result_file,lan='chinese'):
    ...
    if not os.path.exists(audio_result_file):
        tts = gTTS(text=news_str, slow=False, lang='zh-cn')
        tts.save(audio_result_file)

def tts_adjust():
    slow=True
    news_str='最后希望宣萱早日找到生命中的另一半，小仙女都早日脱单，像女神一样青春永驻！'
    tts = gTTS(text=news_str, slow=False,lang='zh-cn')
    audio_path='/media/xiaolong/datapool/data/debug/debug.mp3'
    tts.save(audio_path)

def resize_img():
    from pylab import *
    from scipy.misc import imresize,imsave
    input_path='/media/xiaolong/datapool/data/print1.jpg'
    im=imread(input_path)
    w,h,_=im.shape
    ratio=19.0/13
    im2=imresize(im,[w,int(w*ratio)])
    from PIL import Image
    imsave(input_path.replace('.jpg','_1.jpg'),im2)
...
def generate_news_video_function(pickle_file_path,lan='chinese'):
    ...
    subtitle_file_path=folder_base_path+'subtitle/audio.ass'
    ...
    if not os.path.exists(result_mp4_file.replace('.mp4','_audio.mp4')):    
        video_add_audio_sub_title_fun(audio_result_file,result_mp4_file,subtitle_file_path)
    #import IPython
    #IPython.embed()
...
def generate_news_video_set():
    databe_path='/media/xiaolong/datapool/data/youtube/news/new_mitbbs_3_10/'
